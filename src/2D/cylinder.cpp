...
bool cylinderSpotStain::isOutofRange()
{
    ...
    if ((xip < project->leftboundary + 2) || (xip > project->rightboundary - 2) || (etap < project->downboundary + 2) || (etap > project->upboundary - 2) || ((etap == 0) && (xip >= project->leftterminal) && (xip <= project->rightterminal))) {
        return true;
    } else {
        return false;
    }
...
void cylinderSpotStain::run()
{
    ...
    double neweta;
    uxi = d1 * d2 * project->coordination->access(floor(xip) + 1, floor(etap) + 1).uxi + (1 - d1) * d2 * project->coordination->access(floor(xip), floor(etap) + 1).uxi + d1 * (1 - d2) * project->coordination->access(floor(xip) + 1, floor(etap)).uxi + (1 - d1) * (1 - d2) * project->coordination->access(floor(xip), floor(etap)).uxi;
    ueta = d1 * d2 * project->coordination->access(floor(etap) + 1, floor(etap) + 1).ueta + (1 - d1) * d2 * project->coordination->access(floor(etap), floor(etap) + 1).ueta + d1 * (1 - d2) * project->coordination->access(floor(etap) + 1, floor(etap)).ueta + (1 - d1) * (1 - d2) * project->coordination->access(floor(etap), floor(etap)).ueta;
...
cylinderSpotStainSource::cylinderSpotStainSource(cylinderProject* p):project(p)
{
    int i;
    step = floor(1 / project->density);
    number = ceil((double)(project->upboundary - project->downboundary - 4) / step);
