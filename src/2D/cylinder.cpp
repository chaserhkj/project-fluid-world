    ...
    while (converge) {
        ...
        for (j = downboundary + 2; j < upboundary - 1; j++){
            for (i = leftboundary + 2; i < rightboundary - 1; i++){
                ...
                node = &coordination->access(i, j);
                node->newpsi = (1 - omega) * node->psi + omega * (node->b1 * coordination->access(i + 1, j).psi + node->b2 * coordination->access(i - 1, j).psi + node->b3 * coordination->access(i, j + 1).psi + node->b4 * coordination->access(i, j - 1).psi + node->zeta) / node->b0;
            }
        }
        /* End of one step */
